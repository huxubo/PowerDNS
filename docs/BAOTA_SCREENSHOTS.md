...
```
网站目录：/www/wwwroot/powerdns-api （保持不变）
运行目录：/public （重要：必须设置为 /public）
```

### 具体操作：

- 在 "运行目录" 的下拉框中选择 `/public`
- 如果下拉框中没有 `/public` 选项，可以手动输入 `public`
- 点击 **「保存」** 按钮
...
```
正确配置前：
网站访问路径 → /www/wwwroot/powerdns-api/index.php （❌ 错误）

正确配置后：
网站访问路径 → /www/wwwroot/powerdns-api/public/index.php （✅ 正确）
```
...
**解决方案：**
- 手动输入 `public`（不要加斜杠 `/`）
- 或者在运行目录输入框中输入完整路径：`/www/wwwroot/你的项目名/public`
